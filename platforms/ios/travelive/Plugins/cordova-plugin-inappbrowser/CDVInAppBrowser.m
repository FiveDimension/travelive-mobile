...
- (void)open:(CDVInvokedUrlCommand*)command
{
    ...
    if (url != nil) {
        ...
        } else if ([target isEqualToString:@"_live"]) {
            [self openLive:url withOptions:options];
        } else if ([target isEqualToString:@"_playlive"]) {
            [self openPlayLive:url withOptions:options];
...
- (void)openLive:(NSString*)url withOptions:(NSString*)options
{
    NSLog(@"url:%@, options:%@", url, options);
    dispatch_async(dispatch_get_main_queue(), ^{
        NSData *data = [options dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *optionDir = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
        
        UIStoryboard * stroyboard = [UIStoryboard storyboardWithName:@"Live" bundle:[NSBundle bundleForClass:[PublishViewController class]]];
        PublishViewController * vc = [stroyboard instantiateViewControllerWithIdentifier:@"PublishView"];
        [vc putPublishUrl:url];
        [vc putOption:optionDir];
        [UIApplication sharedApplication].delegate.window.rootViewController = vc;
//        [self.viewController.navigationController pushViewController:vc animated:YES];
    });
}

- (void)openPlayLive:(NSString*)url withOptions:(NSString*)options
{
    NSLog(@"url:%@, options:%@", url, options);
    dispatch_async(dispatch_get_main_queue(), ^{
        NSData *data = [options dataUsingEncoding:NSUTF8StringEncoding];
        ...
        UIStoryboard * stroyboard = [UIStoryboard storyboardWithName:@"Live" bundle:[NSBundle bundleForClass:[PlayViewController class]]];
        PlayViewController * vc = [stroyboard instantiateViewControllerWithIdentifier:@"PlayView"];
        [vc putPlayUrl:url];
        [vc putOption:optionDir];
        [UIApplication sharedApplication].delegate.window.rootViewController = vc;
        //        [self.viewController.navigationController pushViewController:vc animated:YES];
